...
local Misc = {
	...
	INPUT_PROP_MAX			= 0x1f;
	INPUT_PROP_CNT			= 0x1f + 1;	-- (INPUT_PROP_MAX + 1)
}
...
local SyncEvents = {
	...
	SYN_DROPPED		= 3;
	SYN_MAX			= 0xf;
	SYN_CNT			= 0xf + 1;	-- (SYN_MAX+1)
}


local KeysAndButtons = {
	KEY_RESERVED	=0;
	KEY_ESC			=1;
	KEY_1			=2;
	KEY_2			=3;
...
local ForceFeedbackEffectTypes = {
	FF_RUMBLE	= 0x50;
	FF_PERIODIC	= 0x51;
	FF_CONSTANT	= 0x52;
	FF_SPRING	= 0x53;
...
local ForceFeedbackDeviceProperties = {
	FF_GAIN		= 0x60;
	FF_AUTOCENTER	= 0x61;
...
--[[
	Wrapping up for export
--]]
local function copyPairs(src, dst)
	dst = dst or _G;
...
copyPairs(RelativeAxes, Constants)
copyPairs(AbsoluteAxes, Constants)
copyPairs(SwitchEvents, Constants)
copyPairs(MiscEvents, Constants)
copyPairs(LEDs, Constants)
copyPairs(AutoRepeat, Constants)
copyPairs(Sounds, Constants)
copyPairs(IDs, Constants)
